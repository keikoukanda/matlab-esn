classdef DeepESN < handle
    ...
    %This class implements the necessary functionalities for running and
    %training DeepESNs.
    %
    %Reference article:
    ...
    methods (Access = public)
        ...
        self.input_scaling = 0;
        self.inter_scaling = 0;
        self.leaking_rate = 1;
        ...
        % % figure;
        % %
        % % % 各レイヤーの時間平均の最小値と最大値を計算
        % % minVal = min(cellfun(@(x) min(mean(x, 1)), self.l_state));
        % % maxVal = max(cellfun(@(x) max(mean(x, 1)), self.l_state));
        % %
        % % for layer = 1:10
        % %     % 時系列データを取得
        % %     layerData = self.l_state{layer}; % 10x4000 のデータ
        % %
        % %     % 2x5 のサブプロットにプロット
        % %     subplot(5, 2, layer);
        % %
        % %     % 各ニューロンの時間平均を計算
        % %     avg_data = mean(layerData, 1); % 各行の平均を取得することで各ニューロンの時間平均を得る
        % %
        % %     % 各ニューロンの時間平均をプロット
        % %     plot(avg_data, 'LineWidth', 1.5);
        % %
        % %     % xlim([100 300]);
        % %
        % %     % y軸の範囲を手動で設定
        ...
        % % xlabel('neuron');
        % % ylabel('Average Neuron State');
        % % sgtitle('Time-Averaged Neuron State for Each Layer');

        %% vis 2
        % figure;
        % 
        % % 各レイヤーの時間平均と標準偏差を計算
        % minVal = min(cellfun(@(x) min(mean(x, 2)), self.l_state));
        % maxVal = max(cellfun(@(x) max(mean(x, 2)), self.l_state));
        % 
        % for layer = 1:10
        %     % 時系列データを取得
        %     layerData = self.l_state{layer}; % 10x4000 のデータ
        ...
        % 
        %     % 各ニューロンの時間平均と標準偏差を計算
        %     avg_data = mean(layerData, 2); % 各行の平均を取得することで各ニューロンの時間平均を得る
        %     std_data = std(layerData, 0, 2); % 各行の標準偏差を取得することで各ニューロンの時間平均を得る
        % 
        %     % 各ニューロンの時間平均と標準偏差をプロット
        %     errorbar(avg_data, std_data, 'LineWidth', 1.5);
        % 
        %     % y軸の範囲を手動で設定
        %     ylim([minVal - min(std_data), maxVal + max(std_data)]);
        % 
        %     % グラフの装飾
        %     xlabel('Time');
        %     ylabel('Average Neuron State');
        ...
        % ylabel('Average Neuron State');
        % % titleに文字列と変数を組み合わせる
        % title_str = sprintf('Time-Averaged Neuron State for Each Layer (Scaling: %.2f)', self.inter_scaling);
        % 
        % % タイトルを設定
        % sgtitle(title_str);

        %% vis 3 aved neuron then timeseries
        % % figure;
        % 
        % % レイヤーごとにデータを格納するためのセル配列を作成
        % layer_avg_data = cell(1, 10);
        % layer_std_data = cell(1, 10);
        % 
        % % 各レイヤーの時間平均と標準偏差を計算し、データをセル配列に格納
        % for layer = 1:10
        %     % 時系列データを取得
        %     layerData = self.l_state{layer}; % 10x4000 のデータ
        % 
        ...
        %     std_data = std(layerData, 0, 2); % 各行の標準偏差を取得することで各ニューロンの時間平均を得る
        % 
        %     % レイヤーごとの時間平均と標準偏差をセル配列に格納
        %     layer_avg_data{layer} = avg_data;
        %     layer_std_data{layer} = std_data;
        % end
        % 
        % % ニューロン間の平均を取る
        % neuron_avg_data = cellfun(@(x) mean(x, 1), layer_avg_data, 'UniformOutput', false); % 各セルにおける列ごとの平均を計算
        % 
        % % 各レイヤーのニューロン間の平均をプロット
        % errorbar(1:10, cellfun(@mean, neuron_avg_data), cellfun(@std, neuron_avg_data), 'o-', 'LineWidth', 2);
        % 
        % xlabel('Layer', 'FontSize', 25);
        % ylabel('Average Neuron State', 'FontSize', 25);
        % title_str = sprintf('Time-Averaged Neuron State for Each Layer (Scaling: %.2f)', self.inter_scaling);
        % title(title_str, 'FontSize', 25);
        % set(gca, 'FontSize', 25);
        % legend('scaling = 0.1', 'scaling = 1','FontSize', 25);


        %% vis 4 aved timeseries then neuron
        hold on
        % レイヤーごとにデータを格納するためのセル配列を作成
        layer_avg_data = cell(1, 10);
        layer_std_data = cell(1, 10);
        ...
        for layer = 1:10
            ...
            % 各ニューロンの時間平均と標準偏差を計算
            avg_data = mean(layerData, 1); % 各行の平均を取得することで各ニューロンの時間平均を得る
            std_data = std(layerData, 0, 1); % 各行の標準偏差を取得することで各ニューロンの時間平均を得る
        ...
        % ニューロン間の平均を取る
        % 各レイヤーのニューロン間の平均と標準偏差を計算
        neuron_avg_data = cellfun(@(x) mean(x, 2), layer_avg_data, 'UniformOutput', false);
        neuron_std_data = cellfun(@(x) std(x, 0, 2), layer_avg_data, 'UniformOutput', false);

        % 各レイヤーのニューロン間の平均と標準偏差をプロット
        errorbar(1:10, cellfun(@mean, neuron_avg_data), cellfun(@mean, neuron_std_data), 'o-', 'LineWidth', 2);


        %         neuron_avg_data = cellfun(@(x) mean(x, 2), layer_avg_data, 'UniformOutput', false); % 各セルにおける列ごとの平均を計算
        % neuron_std_data = cellfun(@(x) mean(x, 2), layer_std_data, 'UniformOutput', false);
        % % 各レイヤーのニューロン間の平均をプロット
        % errorbar(1:10, cellfun(@mean, neuron_avg_data), cellfun(@std, neuron_std_data), 'o-', 'LineWidth', 2);

        xlabel('Layer', 'FontSize', 25);
        ylabel('Average Neuron State', 'FontSize', 25);
        title_str = sprintf('time ave then neuron ave Neuron State for Each Layer (Scaling: %.2f)', self.inter_scaling);
        title(title_str, 'FontSize', 25);
        set(gca, 'FontSize', 25);
        legend('scaling = 0.1', 'scaling = 1','FontSize', 25);
        grid on
        ...
        if self.readout_regularization == 0
            ...
            writematrix(self.Wout, 'Wout_MC.csv');
        else
